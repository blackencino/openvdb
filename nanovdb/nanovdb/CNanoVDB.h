...
#define DEFINEMASK_int(LOG2DIM, SIZE)                                          \
  ...
  static bool cnanovdb_mask##LOG2DIM##_isOn(                                   \
      const CNANOVDB_GLOBAL cnanovdb_mask##LOG2DIM *RESTRICT mask,             \
      uint32_t n) {                                                            \
...
static void
cnanovdb_map_applyInverse(cnanovdb_Vec3F *dst,
                          const CNANOVDB_GLOBAL cnanovdb_map *RESTRICT map,
                          const cnanovdb_Vec3F *src) {
  ...
  dst->mVec[0] =
      sx * map->mInvMatF[0] + sy * map->mInvMatF[1] + sz * map->mInvMatF[2];
  dst->mVec[1] =
      sx * map->mInvMatF[3] + sy * map->mInvMatF[4] + sz * map->mInvMatF[5];
  dst->mVec[2] =
      sx * map->mInvMatF[6] + sy * map->mInvMatF[7] + sz * map->mInvMatF[8];
}

static void
cnanovdb_map_applyJacobi(cnanovdb_Vec3F *dst,
                         const CNANOVDB_GLOBAL cnanovdb_map *RESTRICT map,
                         const cnanovdb_Vec3F *src) {
  float sx = src->mVec[0];
  float sy = src->mVec[1];
...
static void cnanovdb_map_applyInverseJacobi(
    cnanovdb_Vec3F *dst, const CNANOVDB_GLOBAL cnanovdb_map *RESTRICT map,
    const cnanovdb_Vec3F *src) {
  float sx = src->mVec[0];
  float sy = src->mVec[1];
  float sz = src->mVec[2];
...
static void
cnanovdb_map_applyIJT(cnanovdb_Vec3F *dst,
                      const CNANOVDB_GLOBAL cnanovdb_map *RESTRICT map,
                      const cnanovdb_Vec3F *src) {
  float sx = src->mVec[0];
  float sy = src->mVec[1];
  float sz = src->mVec[2];
  dst->mVec[0] =
      sx * map->mInvMatF[0] + sy * map->mInvMatF[3] + sz * map->mInvMatF[6];
  dst->mVec[1] =
      sx * map->mInvMatF[1] + sy * map->mInvMatF[4] + sz * map->mInvMatF[7];
  dst->mVec[2] =
      sx * map->mInvMatF[2] + sy * map->mInvMatF[5] + sz * map->mInvMatF[8];
}

typedef struct {
  int64_t
      mByteOffset; // byte offset to the blind data, relative to the GridData.
  ...
  uint8_t _reserved[CNANOVDB_ALIGNMENT_PADDING(
      sizeof(int64_t) + sizeof(uint64_t) + 2 * sizeof(uint32_t) +
          2 * sizeof(uint32_t) + 256 * sizeof(char),
...
typedef struct {
  ...
  uint64_t
      mGridSize; // 8B. byte count of this entire grid occupied in the buffer.
  char mGridName[256]; // 256B
  cnanovdb_map mMap;   // 264B. affine transformation between index and world
                       // space in both single and double precision
  ...
  uint32_t _reserved[CNANOVDB_ALIGNMENT_PADDING(
                         8 + 8 + 4 + 4 + 4 + 4 + 8 + 256 + 24 + 24 +
                             sizeof(cnanovdb_map) + 24 + 4 + 4 + 8 + 4,
                         CNANOVDB_DATA_ALIGNMENT) /
                     4];
} cnanovdb_griddata;

static void cnanovdb_griddata_worldToIndex(
    cnanovdb_Vec3F *dst, const CNANOVDB_GLOBAL cnanovdb_griddata *RESTRICT grid,
    const cnanovdb_Vec3F *src) {
  cnanovdb_map_applyInverse(dst, &grid->mMap, src);
}

static void cnanovdb_griddata_indexToWorld(
    cnanovdb_Vec3F *dst, const CNANOVDB_GLOBAL cnanovdb_griddata *RESTRICT grid,
    const cnanovdb_Vec3F *src) {
  cnanovdb_map_apply(dst, &grid->mMap, src);
}

static void cnanovdb_griddata_worldToIndexDir(
    cnanovdb_Vec3F *dst, const CNANOVDB_GLOBAL cnanovdb_griddata *RESTRICT grid,
    const cnanovdb_Vec3F *src) {
  cnanovdb_map_applyInverseJacobi(dst, &grid->mMap, src);
}

static void cnanovdb_griddata_indexToWorldDir(
    cnanovdb_Vec3F *dst, const CNANOVDB_GLOBAL cnanovdb_griddata *RESTRICT grid,
    const cnanovdb_Vec3F *src) {
  cnanovdb_map_applyJacobi(dst, &grid->mMap, src);
}

static void cnanovdb_griddata_applyIJT(
    cnanovdb_Vec3F *dst, const CNANOVDB_GLOBAL cnanovdb_griddata *RESTRICT grid,
    const cnanovdb_Vec3F *src) {
...
typedef struct {
  ...
  uint8_t _reserved[CNANOVDB_ALIGNMENT_PADDING(
      4 * sizeof(uint64_t) + (3 + 3) * sizeof(uint32_t) + sizeof(uint64_t),
      CNANOVDB_DATA_ALIGNMENT)];
} cnanovdb_treedata;

static const CNANOVDB_GLOBAL cnanovdb_treedata *cnanovdb_griddata_tree(
    const CNANOVDB_GLOBAL cnanovdb_griddata *RESTRICT griddata) {
...
static void
cnanovdb_readaccessor_insert(cnanovdb_readaccessor *RESTRICT acc,
                             int childlevel,
...
#define CREATE_LEAF_NODE_int(LEVEL, LOG2DIM, CHILDTOTAL, TOTAL, MASK,          \
                             VALUETYPE, STATSTYPE, SUFFIX)                     \
  typedef struct {                                                             \
    ...
    STATSTYPE mStdDevi;                                                        \
    uint32_t                                                                   \
        _reserved[CNANOVDB_ALIGNMENT_PADDING(                                  \
  ...
  static VALUETYPE cnanovdb_node##LEVEL##SUFFIX##_getValueAndCache(            \
      ...
      const cnanovdb_coord *RESTRICT ijk,                                      \
      cnanovdb_readaccessor                                                    \
          *RESTRICT /* DO NOT REMOVE: Required for C99 compliance */ acc) {    \
    (void)(acc);                                                               \
    uint32_t n = cnanovdb_node##LEVEL##SUFFIX##_CoordToOffset(ijk);            \
    return node->mVoxels[n];                                                   \
  }                                                                            \
...
#define CREATE_LEAF_NODE(LEVEL, LOG2DIM, TOTAL, VALUETYPE, STATSTYPE, SUFFIX)  \
  CREATE_LEAF_NODE_int(LEVEL, LOG2DIM, (TOTAL - LOG2DIM), TOTAL,               \
                       ((1u << TOTAL) - 1u), VALUETYPE, STATSTYPE, SUFFIX)

#define CREATE_INTERNAL_NODE_int(CHILDLEVEL, LEVEL, LOG2DIM, CHILDTOTAL,       \
                                 TOTAL, MASK, VALUETYPE, STATSTYPE, SUFFIX)    \
  typedef struct {                                                             \
    ...
    uint8_t _reserved[CNANOVDB_ALIGNMENT_PADDING(                              \
        sizeof(cnanovdb_mask##LOG2DIM) + sizeof(VALUETYPE) * 2 +               \
            sizeof(STATSTYPE) * 2 + sizeof(cnanovdb_coord) * 2 +               \
  ...
  static const CNANOVDB_GLOBAL cnanovdb_node##CHILDLEVEL##SUFFIX               \
    ...
    const CNANOVDB_GLOBAL cnanovdb_node##CHILDLEVEL##SUFFIX *childnode =       \
        (const CNANOVDB_GLOBAL cnanovdb_node##CHILDLEVEL##SUFFIX               \
             *)(((CNANOVDB_GLOBAL uint8_t *)node) + node->mTable[n].child);    \
    return childnode;                                                          \
  }                                                                            \
                                                                               \
  static VALUETYPE cnanovdb_node##LEVEL##SUFFIX##_getValue(                    \
  ...
  static bool cnanovdb_node##LEVEL##SUFFIX##_isActiveAndCache(                 \
    ...
    if (cnanovdb_mask##LOG2DIM##_isOn(&node->mChildMask, n)) {                 \
      const CNANOVDB_GLOBAL cnanovdb_node##CHILDLEVEL##SUFFIX *child =         \
          cnanovdb_node##LEVEL##SUFFIX##_getChild(node, n);                    \
      cnanovdb_readaccessor_insert(acc, CHILDLEVEL, child, ijk);               \
      return cnanovdb_node##CHILDLEVEL##SUFFIX##_isActiveAndCache(child, ijk,  \
                                                                  acc);        \
    }                                                                          \
    return cnanovdb_mask##LOG2DIM##_isOn(&node->mValueMask, n) ? true : false; \
  }                                                                            \
...
#define CREATE_INTERNAL_NODE(CHILDLEVEL, LEVEL, LOG2DIM, TOTAL, VALUETYPE,     \
                             STATSTYPE, SUFFIX)                                \
  CREATE_INTERNAL_NODE_int(CHILDLEVEL, LEVEL, LOG2DIM, (TOTAL - LOG2DIM),      \
                           TOTAL, ((1u << TOTAL) - 1u), VALUETYPE, STATSTYPE,  \
                           SUFFIX)
...
#define CREATE_ROOTDATA(VALUETYPE, STATSTYPE, SUFFIX)                          \
  typedef struct {                                                             \
    ...
    VALUETYPE value;                                                           \
    uint8_t _reserved[CNANOVDB_ALIGNMENT_PADDING(                              \
        sizeof(KEYSIZE) + sizeof(VALUETYPE) + sizeof(int64_t) +                \
            sizeof(uint32_t),                                                  \
        CNANOVDB_DATA_ALIGNMENT)];                                             \
  ...
  typedef struct {                                                             \
    ...
    STATSTYPE mAverage, mStdDevi;                                              \
    uint32_t _reserved[CNANOVDB_ALIGNMENT_PADDING(                             \
                           sizeof(cnanovdb_coord) * 2 + sizeof(uint32_t) +     \
  ...
  static const CNANOVDB_GLOBAL cnanovdb_rootdata##SUFFIX                       \
      *cnanovdb_treedata_root##SUFFIX(                                         \
          const CNANOVDB_GLOBAL cnanovdb_treedata *RESTRICT treedata) {        \
    return (const CNANOVDB_GLOBAL cnanovdb_rootdata##SUFFIX                    \
                *)((const CNANOVDB_GLOBAL uint8_t *)(treedata) +               \
  ...
  static const CNANOVDB_GLOBAL cnanovdb_rootdata_tile##SUFFIX                  \
    ...
    const CNANOVDB_GLOBAL cnanovdb_rootdata_tile##SUFFIX *basetile =           \
        (const CNANOVDB_GLOBAL cnanovdb_rootdata_tile##SUFFIX *)(rootdata +    \
                                                                 1);           \
    return basetile + n;                                                       \
  }                                                                            \
                                                                               \
  static const CNANOVDB_GLOBAL cnanovdb_node2##SUFFIX                          \
      *cnanovdb_rootdata##SUFFIX##_getChild(                                   \
          const CNANOVDB_GLOBAL cnanovdb_rootdata##SUFFIX *RESTRICT rootdata,  \
          const CNANOVDB_GLOBAL cnanovdb_rootdata_tile##SUFFIX *RESTRICT       \
              tile) {                                                          \
    CNANOVDB_GLOBAL cnanovdb_node2##SUFFIX *basenode =                         \
        (CNANOVDB_GLOBAL cnanovdb_node2##SUFFIX                                \
             *)(((CNANOVDB_GLOBAL uint8_t *)rootdata) + tile->child);          \
    return basenode;                                                           \
  }                                                                            \
                                                                               \
  static const CNANOVDB_GLOBAL cnanovdb_rootdata_tile##SUFFIX                  \
...
inline void cnanovdb_readaccessor_init(
    cnanovdb_readaccessor *RESTRICT acc,
...
#define DEFINE_ISCACHED(LEVEL, MASK)                                           \
  inline bool cnanovdb_readaccessor_isCached##LEVEL(                           \
      cnanovdb_readaccessor *RESTRICT acc, int32_t dirty) {                    \
    if (!acc->mNode[LEVEL])                                                    \
      return false;                                                            \
    if (dirty & ~MASK) {                                                       \
      acc->mNode[LEVEL] = 0;                                                   \
...
inline int32_t
cnanovdb_readaccessor_computeDirty(const cnanovdb_readaccessor *RESTRICT acc,
                                   const cnanovdb_coord *RESTRICT ijk) {
...
#define CREATE_ACCESSOR(VALUETYPE, SUFFIX)                                     \
  inline VALUETYPE cnanovdb_readaccessor_getValue##SUFFIX(                     \
      cnanovdb_readaccessor *RESTRICT acc,                                     \
      const cnanovdb_coord *RESTRICT ijk) {                                    \
    ...
    if (cnanovdb_readaccessor_isCached1(acc, dirty))                           \
      return cnanovdb_node1##SUFFIX##_getValueAndCache(                        \
          ((CNANOVDB_GLOBAL cnanovdb_node1##SUFFIX *)acc->mNode[1]), ijk,      \
          acc);                                                                \
    if (cnanovdb_readaccessor_isCached2(acc, dirty))                           \
      return cnanovdb_node2##SUFFIX##_getValueAndCache(                        \
          ((CNANOVDB_GLOBAL cnanovdb_node2##SUFFIX *)acc->mNode[2]), ijk,      \
          acc);                                                                \
                                                                               \
    return cnanovdb_rootdata##SUFFIX##_getValueAndCache(                       \
        ((CNANOVDB_GLOBAL cnanovdb_rootdata##SUFFIX *)acc->mNode[3]), ijk,     \
        acc);                                                                  \
  }                                                                            \
                                                                               \
  inline bool cnanovdb_readaccessor_isActive##SUFFIX(                          \
      cnanovdb_readaccessor *RESTRICT acc,                                     \
      const cnanovdb_coord *RESTRICT ijk) {                                    \
    int32_t dirty = cnanovdb_readaccessor_computeDirty(acc, ijk);              \
                                                                               \
...
static int cnanovdb_griddata_valid(
...
static int cnanovdb_griddata_validF(
    const CNANOVDB_GLOBAL cnanovdb_griddata *RESTRICT grid) {
...
static int cnanovdb_griddata_validF3(
    const CNANOVDB_GLOBAL cnanovdb_griddata *RESTRICT grid) {
  if (!cnanovdb_griddata_valid(grid))
    return 0;
